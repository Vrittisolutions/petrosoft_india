...
class cashDetails extends StatefulWidget{
  const cashDetails({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return cashDetailsState();
  }

}
class cashDetailsState extends State<cashDetails>{
  ...
  final TextEditingController finalTotalValue=TextEditingController(text: '0.00');
  ...
  Future<bool?> _willPopCallback()async{
    ...
    return Navigator.of(context)
      ...
      return PetroSoftOperatorHomePage();
    }));
  ...
  Widget build(BuildContext context) {
                                          ...
                                          RichText(
                                            text: TextSpan(
                                              ...
                                              color: ColorsForApp.appThemeColor
                                                ...
                                                children: <TextSpan>[
                                                  TextSpan(text: UT.m["cash_balance"] ,
                                                    style:TextStyle(
                                                      //fontFamily: SavangadiAppTheme.fontName,
                            ...
                            Flexible(
                              child: SizedBox(
                                ...
                                child: TextField(
                                    ...
                                    readOnly: true,
                                    style: TextStyle(fontSize: 13,color: ColorsForApp.secondary,fontWeight: FontWeight.w600) ,
                                    decoration: InputDecoration(
                                      labelText: "Note Rs.",
                                      labelStyle: TextStyle(fontSize: 15,color: ColorsForApp.appThemeColor,fontWeight: FontWeight.w600),
   ...
   /* var rs = (Id2Flt("rs_500") * 500) + (Id2Flt("rs_200") * 200) + (Id2Flt("rshund") * 100) + (Id2Flt("rsfifty") * 50);
    rs += (Id2Flt("rs_2000") * 2000) + (Id2Flt("rs_20") * 20) + (Id2Flt("rsten") * 10);
    rs += (Id2Flt("rsfive") * 5) + (Id2Flt("rstwo") * 2) + (Id2Flt("_coin10") * 10) + (Id2Flt("_coin5") * 5) + Flt(Id2Flt("rsone")) + Flt(Id2Flt("coins"));*/
    ...
    setState(() {
      ...
      finalTotalValue.text=rs.toString();
  ...
  getCashierWiseData() async {
     var _urlHD = UT.APIURL! +
         "api/PumpTrnEgenShift/Getdenomition?year=" +
         UT.curyear! + "&shop=" + UT.shop_no! +
         "&dsr_no=${UT.m["dSrNo"]}&cashier_ac=${UT.ClientAcno}&Addrow=true";
     print("_urlHD-->$_urlHD");
     cashierHD = await UT.apiDt(_urlHD);
     print("cash Hd-->$cashierHD");
    // cashBalance=PumpHd[0]['cashbal'].toStringAsFixed(2);
     rs2000xTxt.text=cashierHD[0]['rs2000'].toStringAsFixed(2);
     rs500xTxt.text=cashierHD[0]['rs500'].toStringAsFixed(2);
     rs200xTxt.text=cashierHD[0]['rs200'].toStringAsFixed(2);
     rs100xTxt.text=cashierHD[0]['rs100'].toStringAsFixed(2);
     rs50xTxt.text=cashierHD[0]['rs50'].toStringAsFixed(2);
     rs20xTxt.text=cashierHD[0]['rs20'].toStringAsFixed(2);
     rs10xTxt.text=cashierHD[0]['rs10'].toStringAsFixed(2);
     rs5xTxt.text=cashierHD[0]['rs5'].toStringAsFixed(2);
     coin10xTxt.text=cashierHD[0]['coin10'].toStringAsFixed(2);
     coin5xTxt.text=cashierHD[0]['coin5'].toStringAsFixed(2);
     coin2xTxt.text=cashierHD[0]['rs2'].toStringAsFixed(2);
     coin1xTxt.text=cashierHD[0]['rs1'].toStringAsFixed(2);
    // coinTxt.text=cashierHD[0]['coins'].toStringAsFixed(2);
     rstotal();
   }
   ...
   saveData() async {
     cashierHD[0]['srno']=UT.m["dSrNo"];
     cashierHD[0]['date']=UT.yearMonthDateFormat(UT.m['saledate']);
     cashierHD[0]['shift']=UT.m['shift'];
     cashierHD[0]['rs2000']=rs2000xTxt.text;
     cashierHD[0]['rs500']=rs500xTxt.text;
     cashierHD[0]['rs200']=rs200xTxt.text;
     cashierHD[0]['rs100']=rs100xTxt.text;
     cashierHD[0]['rs50']=rs50xTxt.text;
     cashierHD[0]['rs20']=rs20xTxt.text;
     cashierHD[0]['rs10']=rs10xTxt.text;
     cashierHD[0]['rs5']=rs5xTxt.text;
     cashierHD[0]['coin10']=coin10xTxt.text;
     cashierHD[0]['coin5']=coin5xTxt.text;
     cashierHD[0]['rs2']=coin2xTxt.text;
     cashierHD[0]['rs1']=coin1xTxt.text;
    // cashierHD[0]['rec_amt']=finalTotalValue.text;
     cashierHD[0]['cash_total']=finalTotalValue.text;
     //cashierHD[0]['coins']=coinTxt.text;

     print("Updating HD->$cashierHD");
     var _url1 = UT.APIURL! +
         "api/PostData/Post?tblname=crcd" +
         UT.curyear!+ UT.shop_no!;
     _url1 += "&Unique=dsr_no,cashier_ac";

     print("pmhd-url->$_url1");
     var res = await UT.save2Db(_url1, cashierHD);
     print("PumpHd-res->$res");
     if(res=="ok"){
       UT.m["cashUpdated"]="Yes";
