...
class AddPaymentEntry extends StatefulWidget {
  ...
  final Map selectedRow;
    AddPaymentEntry({Key? key, required this.voucherNo,required this.mode,required this.selectedRow}) : super(key: key);
   @override
...
class _AddPaymentEntryState extends State<AddPaymentEntry> {
  ...
  void initState() {
    ...
    getBankList();
    if(widget.mode=="Edit"){
      print('payment selectedRow --->${widget.selectedRow}');
      voucherNoController.text=widget.selectedRow['srno'].toString();
      amountController.text=widget.selectedRow['amount'].toString();
      discountController.text=widget.selectedRow['disc_amt'].toString();
      UT.m["Selected_ACCNO"]=widget.selectedRow['acno'].toString();
      narrationController.text=widget.selectedRow['narr1'].toString();
      _ledgerAccountName=widget.selectedRow['byname'].toString();
      UT.m["Selected_bankno"]=widget.selectedRow['bytoacno'].toString();
      _selectedPaymentMode=widget.selectedRow['mode'];
      isdeleted=widget.selectedRow['isdeleted'].toString();
      date=widget.selectedRow['date'];
      UT.m["Selected_bankno"]=widget.selectedRow['acno'];
    }else{
      date=UT.dateMonthYearFormat(UT.m['saledate']);
  ...
  getBankList() async {
    ...
    if(widget.mode=="Edit"){
      for(int i=0;i<bankList.length;i++){
        bankName=UT.GetRow(bankList, 'acno', widget.selectedRow['bytoacno'], 'name');
    ...
    checkNewSrNo().then((value) async {
      if(value!=null){
        ...
        if(Result1=="ok"){
          ...
          Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => ManagerPaymentEntry()));
        }else{
          DialogBuilder(context).hideOpenDialog();
  ...
  saveEditedData() async {
    ...
    checkNewSrNo().then((value) async {
      if(value!=null){
        ...
        paymentData["mode"]=_selectedPaymentMode;
        paymentData["chqno"]=checkNoController.text;
        paymentData["isdeleted"]=isdeleted;
