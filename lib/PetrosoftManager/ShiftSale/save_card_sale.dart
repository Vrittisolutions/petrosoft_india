...
class ManagerSaveCardSale extends StatefulWidget {
  final String mode;
  final Map selectedRow;
  const ManagerSaveCardSale({Key? key, required this.mode, required this.selectedRow}) : super(key: key);

  @override
  _ManagerSaveCardSaleState createState() => _ManagerSaveCardSaleState();
}

class _ManagerSaveCardSaleState extends State<ManagerSaveCardSale> {
  ...
  void initState() {
    ...
    if(widget.mode=="Edit") {
      ...
      UT.m["Selected_cashierAcNo"]=widget.selectedRow['pump_hold'];
  ...
  shiftInChargeData() async {
    ...
    //print("cashierList-->$cashierList");
    if(widget.mode=="Edit"){
      for(int i=0;i<cashierList.length;i++){
        cashierName = UT.GetRow(cashierList, "acno", widget.selectedRow['pump_hold'].trim(),'name');
 ...
 Widget saveButton(){
  ...
  postData() async {
    ...
    cardSaleData["pump_hold"]=UT.m["Selected_cashierAcNo"];
    ...
    if(result=="ok"){
      ...
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const ManagerCreditCardSale()));
    }else{
      DialogBuilder(context).hideOpenDialog();
