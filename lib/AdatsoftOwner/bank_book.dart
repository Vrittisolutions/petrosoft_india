...
class BankBook extends StatefulWidget {
  const BankBook({Key? key}) : super(key: key);

  @override
  _BankBookState createState() => _BankBookState();
}
...
class _BankBookState extends State<BankBook> {
  ...
  getBankList() async {
    var _url = UT.APIURL! +
        "api/AccountMaster/getBankList?shop2getBank=" +
        UT.shop_no!;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const CommonAppBarText(title: 'Bank Book',),
      ),
     // appBar: AppBar(title: const Text('Bank Book',), backgroundColor: ColorsForApp.appThemeColorAdatOwner),
      body: Padding(
      ...
      bottomNavigationBar: CommonButtonForAllApp(onPressed: () async {
       // rptbase64pdf = GetStr(APIURL + "/api/BankBook?shop=" + Firm("shop") + "&FromDt=" + yymmdd($("#date1").val()) + "&Todt=" + yymmdd($("#date2").val()) + "&bytoacc_no=" + selectedAcno + "&getPrint=true");
       if(bankName==null||bankName==""){
         ...
         var _url = UT.APIURL! +
             "api/BankBook?shop=" +
             UT.shop_no.toString()+
             "&FromDt="+"${UT.dateConverter(selectedFromDate)}"+"&Todt="+"${UT.dateConverter(selectedToDate)}"+"&bytoacc_no=${UT.m["Selected_bankno"]}"+"&getPrint=true";
         Navigator.push(
             context,
             MaterialPageRoute(
                 builder: (context) =>
