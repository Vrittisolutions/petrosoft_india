...
class CashCreditSaleReport extends StatefulWidget {
  const CashCreditSaleReport({Key? key}) : super(key: key);

  @override
  _CashCreditSaleReportState createState() => _CashCreditSaleReportState();
}

class _CashCreditSaleReportState extends State<CashCreditSaleReport> {
  TextEditingController balanceController=TextEditingController(text: "0.00");
  dynamic selectedReportType = "All";
  ...
  List<DropdownMenuItem<String>> reportType = [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const CommonAppBarText(title: 'Cash Credit Sale  Report'),
      ),
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: <Widget>[
                      Text("Report Type  ",style: StyleForApp.text_style_bold_14_black),
                      Container(
                          height: 40,width: 150,
                          decoration: BoxDecoration(
                          ...
                          child: Padding(
                            ...
                            child: DropdownButton(
                              ...
                              onChanged: (val) async {
                                selectedReportType = val.toString();
                                setState(() {});
                              },
                            ),
                          )),
      ...
      bottomNavigationBar: CommonButtonForAllApp(
          onPressed: (){
            //rptbase64pdf = GetStr(APIURL + "/api/Salerep01D?shop=" + Firm("shop") + "&date1=" + yymmdd($("#date1").val()) + "&date2=" + yymmdd($("#date2").val()) + "&repType=" + $("#repType").val());
            var _url = UT.APIURL! +
                  "api/Salerep01D?shop=" +
                  UT.shop_no.toString()+
                  "&date1="+"${UT.dateConverter(currentDate)}"+"&date2="+"${UT.dateConverter(selectedToDate!)}"+"&repType=$selectedReportType";
