...
class PattiDetails extends StatefulWidget {
  const PattiDetails({Key? key}) : super(key: key);

  @override
  _PattiDetailsState createState() => _PattiDetailsState();
}

class _PattiDetailsState extends State<PattiDetails> {
  dynamic paymentData;
  dynamic api;
  ...
  getData() async {
    ...
    for(int i=0;i<paymentData.length;i++){
      totalCredit+=paymentData[i]["credit"];
    }
    setState(() {});
    return paymentData;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: CustomAppBarWithoutTrailing(title: 'Patti Details',onPressed: (){
              ...
              Align(
                ...
                child:   Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      Align(alignment:Alignment.center,child: Text("Total Bal : \u{20B9}${totalCredit.toStringAsFixed(2)}",style: const TextStyle(fontSize:15,fontWeight: FontWeight.bold,color: Colors.white),)),
                    ],
                  ),
                ),//last one
              )
